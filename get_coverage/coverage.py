...
def calculate_nc(model, image, threshold=0.6):
    device = next(model.parameters()).device
    model.eval()
    image = image.unsqueeze(0).to(device)
    layer_output_dict = tool.get_layer_output(model, image)
    nc_coverage = {}
    for layer_name, output in layer_output_dict.items():
        scaled_output = tool.scale(output)
        nc_coverage[layer_name] = (scaled_output > threshold).float().mean().item()
...
class nc:
    ...
    def calculate_single_image_nc(self, image):

        image = image.unsqueeze(0).to(self.device)
        layer_output_dict = tool.get_layer_output(self.model, image)
        nc_coverage = {}
        for layer_name, output in layer_output_dict.items():
            scaled_output = tool.scale(output)
            nc_coverage[layer_name] = (scaled_output > self.threshold).float().mean().item()
