...
def _vgg(arch, cfg, batch_norm, pretrained, progress, device, **kwargs):
    ...
    if pretrained:
        ...
        # )
        script_dir = "/data/user/collection/CIFAR-10/CLASSIFIER/"
        state_dict = torch.load(
...
def vgg11_bn(pretrained=False, progress=True, device="cuda", **kwargs):
...
def vgg13_bn(pretrained=False, progress=True, device="cuda", **kwargs):
...
def vgg16_bn(pretrained=False, progress=True, device="cuda", **kwargs):
...
def vgg19_bn(pretrained=False, progress=True, device="cuda", **kwargs):
